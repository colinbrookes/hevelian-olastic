...
public class BucketsAggregationsRequestCreator extends AbstractAggregationsRequestCreator {
	...
	public AggregateRequest create(UriInfo uriInfo) throws ODataApplicationException {
		...
		List<AggregationBuilder> bucketsQueries = getBucketsQueries(groupByItems.get(0), entityType, pagination);

		AggregateQuery aggregateQuery = new AggregateQuery(baseQuery.getIndex(), baseQuery.getType(),
				baseQuery.getQueryBuilder(), bucketsQueries, Collections.emptyList());
	 ...
	 */
	protected List<AggregationBuilder> getBucketsQueries(GroupBy groupBy, ElasticEdmEntityType entityType,
			Pagination pagination) throws ODataApplicationException {
		...
		TermsAggregationBuilder groupByQuery = AggregationBuilders.terms(lastField)
				.field(addKeywordIfNeeded(lastField, entityType)).size(pagination.getSkip() + pagination.getTop());
		getMetricsAggQueries(getAggregations(groupBy.getApplyOption()), entityType)
				.forEach(groupByQuery::subAggregation);
		for (String field : fields) {
			groupByQuery = AggregationBuilders.terms(field).field(addKeywordIfNeeded(field, entityType))
