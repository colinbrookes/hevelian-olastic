 ...
 * @author rdidyk
 */
public class BucketsAggregationsParser extends AbstractParser<EdmEntityType, AbstractEntityCollection> {
	...
	public InstanceData<EdmEntityType, AbstractEntityCollection> parse(SearchResponse response,
		...
		List<Entity> entityList = getAggregatedEntities(response.getAggregations().asMap(), null, entityType);
	 ...
	 *            the edm entity type
	 */
	private void addAggsAndCountIfNeeded(Map<String, Aggregation> aggregations, long count, Entity entity,
			ElasticEdmEntityType entityType) {
		aggregations.entrySet().stream().filter(e -> e.getValue() instanceof SingleValue)
				.map(agg -> createProperty(agg.getKey(), ((SingleValue) agg.getValue()).value(), entityType))
