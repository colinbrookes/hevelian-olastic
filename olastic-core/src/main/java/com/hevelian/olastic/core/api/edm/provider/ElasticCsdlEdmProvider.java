...
public abstract class ElasticCsdlEdmProvider extends CsdlAbstractEdmProvider {
	...
	private static final FullQualifiedName DEFAULT_CONTAINER_NAME = new FullQualifiedName("OData", "ODataService");
	 ...
	 *            ES to CSDL mapper
	 */
	public ElasticCsdlEdmProvider(MappingMetaDataProvider metaDataProvider, ElasticToCsdlMapper csdlMapper) {
	 ...
	 *            nested type mapper
	 */
	public ElasticCsdlEdmProvider(MappingMetaDataProvider metaDataProvider, NestedTypeMapper nestedTypeMapper) {
	 ...
	 */
	@Override
	public ElasticCsdlEntityType getEntityType(FullQualifiedName entityTypeName) throws ODataException {
		...
		entityType
				.setProperties(getProperties(entityTypeName, mappingMetaDataProvider.getMappingForType(eIndex, eType)));
	 ...
	 */
	protected List<CsdlProperty> getProperties(FullQualifiedName entityTypeName, MappingMetaData metaData)
		...
		try {
			...
			for (String eFieldName : eTypeProperties.map.keySet()) {
				...
				properties.add(
						new ElasticCsdlProperty().setEIndex(eIndex).setEType(eType).setEField(eFieldName).setName(name)
								.setType(type).setCollection(csdlMapper.eFieldIsCollection(eIndex, eType, eFieldName)));
			}
			return properties;
		} catch (IOException e) {
	 ...
	 */
	protected List<CsdlNavigationProperty> getNavigationProperties(FullQualifiedName entityTypeName) {
		...
		for (ObjectObjectCursor<String, FieldMappingMetaData> e : eFieldMappings) {
			...
			if (eType.equals(eParentType)) {
				CsdlNavigationProperty navProp = new CsdlNavigationProperty()
						.setName(csdlMapper.eChildRelationToNavPropName(index, e.key, entityTypeName.getName()))
			...
			if (eType.equals(e.key)) {
				CsdlNavigationProperty navProp = new CsdlNavigationProperty()
						.setName(csdlMapper.eParentRelationToNavPropName(index, eParentType, entityTypeName.getName()))
						.setType(csdlMapper.eTypeToEntityType(index, eParentType)).setNullable(false)
						.setPartner(csdlMapper.eChildRelationToNavPropName(index, e.key,
								new FullQualifiedName(namespace, eParentType).getName()));
				navPropList.add(navProp);
			}
		}
		return navPropList;
	...
	@Override
	public ElasticCsdlEntitySet getEntitySet(FullQualifiedName entityContainer, String entitySetName)
			throws ODataException {
		...
		for (CsdlNavigationProperty property : navigationProperties) {
			...
			navPropBinding.setTarget(csdlMapper.eTypeToEntitySet(namespaceToIndex(property.getTypeFQN().getNamespace()),
	 ...
	 */
	protected FullQualifiedName entitySetToEntityType(FullQualifiedName entityContainer, String entitySetName)
			throws ODataException {
	...
	protected List<CsdlEntityType> getEntityTypes(String index) throws ODataException {
		...
		for (ObjectCursor<String> key : mappingMetaDataProvider.getAllMappings(index).keys()) {
			CsdlEntityType entityType = getEntityType(csdlMapper.eTypeToEntityType(index, key.value));
	 ...
	 */
	protected CsdlEntityContainer getEntityContainerForSchema(String namespace) throws ODataException {
		CsdlEntityContainer entityContainer = new CsdlEntityContainer();
		entityContainer.setName(getContainerName().getName());

		List<CsdlEntitySet> entitySets = new ArrayList<>();
		for (ObjectCursor<String> key : mappingMetaDataProvider.getAllMappings(namespaceToIndex(namespace)).keys()) {
			entitySets.add(getEntitySet(new FullQualifiedName(namespace, getContainerName().getName()), key.value));
		}
	...
	@Override
	public ElasticCsdlComplexType getComplexType(FullQualifiedName complexTypeName) throws ODataException {
