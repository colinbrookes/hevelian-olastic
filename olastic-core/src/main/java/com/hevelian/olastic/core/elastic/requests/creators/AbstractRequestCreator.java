...
public abstract class AbstractRequestCreator implements ESRequestCreator {
	...
	public BaseRequest getBaseRequestInfo(UriInfo uriInfo) throws ODataApplicationException {
		...
		ElasticEdmEntitySet responseEntitySet = (ElasticEdmEntitySet) getFirstResourceEntitySet(uriInfo);
		...
		while (iterator.hasNext()) {
			...
			if (segment.getKind() == UriResourceKind.navigationProperty) {
				responseEntitySet = getNavigationTargetEntitySet(responseEntitySet, (UriResourceNavigation) segment);
		...
		return new BaseRequest(
				new Query(responseEntitySet.getEIndex(), responseEntitySet.getEType(), queryBuilder.build()),
				responseEntitySet);
	}
	 ...
	 * @throws ODataApplicationException
	 */
	protected EdmEntitySet getFirstResourceEntitySet(UriInfo uriInfo) throws ODataApplicationException {
		List<UriResource> resourceParts = uriInfo.getUriResourceParts();
	...
	protected BoolQueryBuilder getFilterQuery(UriInfo uriInfo) throws ODataApplicationException {
		...
		try {
			if (filterOption != null) {
				...
				QueryBuilder query = ((ExpressionResult) expression.accept(new ElasticSearchExpressionVisitor()))
						.getQueryBuilder();
				...
				for (Expression expression : expressions) {
					filterQuery.filter((QueryBuilder) expression.accept(new ElasticSearchExpressionVisitor()));
				}
			}
	...
	protected BoolQueryBuilder getSearchQuery(UriInfo uriInfo) throws ODataApplicationException {
		...
		if (searchOption != null) {
			...
			List<SearchExpression> expressions = ApplyOptionUtils.getSearchItems(applyOption).stream()
					.map(e -> e.getSearchOption().getSearchExpression()).collect(Collectors.toList());
	...
	private void buildSearchQuery(BoolQueryBuilder parentQuery, SearchExpression expression) {
		if (expression.isSearchBinary()) {
			...
			parentQuery.mustNot(matchQuery(ElasticConstants.ALL_FIELD, unary.getOperand().getSearchTerm()));
		} else {
			parentQuery.must(matchQuery(ElasticConstants.ALL_FIELD, expression.asSearchTerm().getSearchTerm()));
		}
	}
				...
				if (expression instanceof Member) {
					...
					if (oUriResource instanceof UriResourcePrimitiveProperty) {
						ElasticEdmProperty edmProperty = (ElasticEdmProperty) ((UriResourcePrimitiveProperty) oUriResource)
								.getProperty();
						orderBy.add(new Sort(addKeywordIfNeeded(edmProperty.getEField(), edmProperty.getType()),
