...
public class MemberHandler {
     ...
     * @param collectionResourceCache cache with parent members collection resources. Used for nested lambdas
     * @return expression member
     * @throws ODataApplicationException
     * @throws ExpressionVisitException
     */
    public ExpressionMember handle(Map collectionResourceCache) throws ODataApplicationException, ExpressionVisitException {
        ...
        } else if (lastPart instanceof UriResourcePrimitiveProperty || lastPart instanceof UriResourceLambdaVariable) {
    ...
    private ExpressionMember handleLambdaAny()
        ...
        // complex or primitive type collection
            ...
            setPath(expression);
            //if any lambda uses primitive property (//Books?$filter=property/any(p:p eq 'value'))
            ...
            String nestedPath = isPreLastResourcePrimitive() ?
                    StringUtils.substringBeforeLast(pathToMember, ElasticConstants.NESTED_PATH_SEPARATOR) : pathToMember;

            ExpressionResult expressionResult = (ExpressionResult) expression
                    .accept(visitor);
            return isPreLastResourcePrimitive()
            ? expressionResult
    ...
    private ExpressionMember handlePrimitive() {
        ...
        // Books?$filter=nested/any(n:n/state eq true)
            ...
            EdmProperty lastProperty = ((UriResourceProperty) lastPart).getProperty();
            String parentPathPrefix = pathToMember != null ? pathToMember + ElasticConstants.NESTED_PATH_SEPARATOR : "";
        ...
        //Books?$filter=nested/property/any(p:p/tags/any(t:t eq 'Tag'))
                ...
                && ((UriResourcePartTyped) firstPart).getType().getKind() == EdmTypeKind.PRIMITIVE) {
        ...
        // simple primitive expression or expression inside lambda for retrieving children
        else {
            EdmProperty lastProperty = ((UriResourceProperty) lastPart).getProperty();
     ...
     * We need to store this path manually because Member inside lambda doesn't contain full path to itself
     *
    ...
    public UriResource getCollectionResource() {
        ...
        if (lastPart.getKind() == UriResourceKind.lambdaAll || lastPart.getKind() == UriResourceKind.lambdaAny){
