...
public class TestProvider extends ElasticCsdlEdmProvider {
	...
	public ElasticCsdlEntityType getEntityType(FullQualifiedName entityTypeName) throws ODataException {
		CsdlProperty id = new ElasticCsdlProperty().setName("_id")
				.setType(EdmPrimitiveTypeKind.String.getFullQualifiedName());
		CsdlProperty dimensionProperty = new ElasticCsdlProperty().setName(DIMENSION_TYPE).setType(DIMENSION_FQN)
				.setCollection(true);
		...
		if (entityContainer.equals(CONTAINER)) {
			if (entitySetName.equals(AUTHOR_TYPE)) {
				...
				entitySet.setNavigationPropertyBindings(
						Arrays.asList(new CsdlNavigationPropertyBinding().setPath(ADDRESS_TYPE).setTarget(ADDRESS_TYPE),
				...
				entitySet.setNavigationPropertyBindings(
						Arrays.asList(new CsdlNavigationPropertyBinding().setPath(AUTHOR_TYPE).setTarget(AUTHOR_TYPE)));
				return entitySet;
				...
				entitySet.setNavigationPropertyBindings(
						Arrays.asList(new CsdlNavigationPropertyBinding().setPath(BOOK_TYPE).setTarget(BOOK_TYPE)));
				return entitySet;
			}
		}
		return null;
